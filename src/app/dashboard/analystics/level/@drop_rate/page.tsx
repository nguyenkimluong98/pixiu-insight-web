...
import { useDropRateStats } from '@/features/analystics/level/api';
...
import { z } from 'zod';
import DropRateLineChart from '@/features/analystics/level/components/drop-rate-line-chart';
import { dropRateColumns } from '@/features/analystics/level/components/stats-tables/columns';
import { DropRate } from '@/types/level';
import { BarChart, LineChart } from 'lucide-react';
import { Switch } from '@/components/ui/switch';
import DropRateBarChart from '@/features/analystics/level/components/drop-rate-bar-chart';
...
export default function DropRateChart() {
  ...
  const { data, loading, error, execute } = useDropRateStats();

  useEffect(() => {
    if (!isFilterReady || activeTab !== Tabs.DROP_RATE) return;
  ...
  const filteredData = useFilteredStats<DropRate>(data?.data || [], query);
    ...
    <Card>
      ...
      <CardHeader>
        <CardTitle className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5'>
          <div className='col-span-3'>
            <span>Player Drop Rate by Max Level</span>
      ...
      <CardContent>
        {!loading && (
          <div>
            {isBarChartSelect ? (
              <DropRateBarChart data={data?.data} />
            ) : (
              <DropRateLineChart data={data?.data} />
            )}
            <StatsTable
              data={filteredData || []}
              columns={dropRateColumns}
